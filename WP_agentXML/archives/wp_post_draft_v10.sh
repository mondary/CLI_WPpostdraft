...
# WordPress XML-RPC Draft Poster Script V4 Clean
# Stable version without auto-scheduling
...
show_usage() {
    ...
    echo "  -s, --schedule      Enable auto-scheduling (creates scheduled posts)"
    ...
    echo "  $0 --auto                   # Auto-detect config and create draft"
    echo "  $0 --auto --schedule        # Auto-detect config and create scheduled post"
        ...
        <param>
            <value>
                <struct>
                    ...
                    </member>")$([ -n "$featured_image_url" ] && echo "
                    <member>
                                ...
                                <data>
                                    ...
                                    <value>
                                        <struct>
                                            <member>
                                                <name>key</name>
                                                <value><string>no_auto_schedule</string></value>
                                            </member>
                                            <member>
                                                <name>value</name>
                                                <value><string>1</string></value>
...
main() {
    ...
    local payload=$(create_post_payload "$POST_TITLE" "$POST_CONTENT" "$POST_SLUG" "$WP_USERNAME" "$WP_PASSWORD" "$FEATURED_IMAGE_ID" "$IMAGE_URL")
    ...
    if [ -n "$post_id" ]; then
        if [ "$SCHEDULE_MODE" = true ]; then
            echo -e "${GREEN}Post created and scheduled successfully!${NC}"
        else
        ...
        echo "Slug: $POST_SLUG"
        if [ "$SCHEDULE_MODE" = true ]; then
            echo "Status: Scheduled for next weekday at 14:00"
            echo "Auto-Schedule: Enabled"
        else
            echo "Status: Draft"
            echo "Auto-Schedule: Disabled"
        fi
        if [ -n "$IMAGE_URL" ]; then
        ...
        echo ""
        if [ "$SCHEDULE_MODE" = true ]; then
            echo -e "${GREEN}Your post will be automatically scheduled!${NC}"
            echo -e "${YELLOW}Note: Save the post in WordPress admin to trigger scheduling${NC}"
        else
