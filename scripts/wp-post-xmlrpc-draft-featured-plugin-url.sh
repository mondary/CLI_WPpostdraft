...
#
# WordPress XML-RPC draft publisher (featured via plugin URL meta)
...
# Gérer l'image featured (URL ou upload) avec support "Featured Image by URL"
handle_featured_image() {
    ...
    if [[ "$image_path" =~ ^https?:// ]]; then
        if [[ "$VERBOSE" == true ]]; then
            ...
            echo "  Mode URL: utilisation du plugin 'Featured Image by URL'" >&2
        fi
        # Retourner l'URL avec un marqueur spécial
        echo "FEATURED_URL:$image_path"
        return 0
    else
        ...
        if [[ -n "$post_id" && "$post_id" =~ ^[0-9]+$ ]]; then
            ...
            if [[ -n "$image_path" ]]; then
                ...
                if image_result=$(handle_featured_image "$image_path"); then
                    local image_result_clean
                    image_result_clean="$(echo "$image_result" | tail -n1)"
                    if [[ "$image_result_clean" =~ ^FEATURED_URL: ]]; then
                        # C'est une URL - utiliser le plugin Featured Image by URL
                        local image_url="${image_result_clean#FEATURED_URL:}"
                        echo "  ✓ URL d'image featured détectée: $image_url"
                        echo "  🔗 Définition via plugin 'Featured Image by URL'..."
                        
                        if set_featured_image_url "$post_id" "$image_url"; then
                            echo "  ✅ Image featured URL définie avec succès"
                        else
                            echo "  ⚠ Avertissement: Échec de la définition de l'image featured URL" >&2
                            echo "    Vérifiez que le plugin 'Featured Image by URL' est installé et activé" >&2
                        fi
                    else
                        # C'est un ID d'attachment (upload réussi)
                        if set_featured_image "$post_id" "$image_result_clean"; then
                            echo "  ✓ Image featured uploadée et définie avec succès (ID: $image_result_clean)"
                        else
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        if [[ -n "$IMAGE" ]]; then
            ...
            if [[ "$IMAGE" =~ ^https?:// ]]; then
                ...
                echo "  💡 L'URL sera définie comme featured image via le plugin 'Featured Image by URL'"
            elif [[ -f "$IMAGE" ]]; then
